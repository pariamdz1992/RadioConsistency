"""
Train a conditioned consistency model on radio maps.
Based on original cm_train.py but modified for spatial conditioning.
"""

import argparse

from cm import dist_util, logger
from cm.radio_datasets import load_data  # Use your radio dataset loader
from cm.resample import create_named_schedule_sampler
from cm.script_util_conditioned import (  # Use conditioned script utilities
...
def main():
    ...
    logger.log("creating conditioned model and diffusion...")
    ...
    # Create conditioned model (this now uses ConditionedConsistencyUNet)
    ...
    logger.log("creating radio data loader...")
    ...
    # MODIFIED: Use radio dataset loader with conditioning
    data = load_data(
        ...
        use_image_conditioning=True,  # Enable conditioning
        dataset_type=getattr(args, 'dataset_type', 'RadioUNet_c'),
        simulation=getattr(args, 'simulation', 'DPM'),
        carsSimul=getattr(args, 'carsSimul', 'no'),
        carsInput=getattr(args, 'carsInput', 'no'),
        cityMap=getattr(args, 'cityMap', 'complete'),
        missing=getattr(args, 'missing', 1),
        numTx=getattr(args, 'numTx', 80),
        thresh=getattr(args, 'thresh', 0.05),
        IRT2maxW=getattr(args, 'IRT2maxW', 1),
    )

    # Teacher model handling (same as original)
    if len(args.teacher_model_path) > 0:
    ...
    # Target model for distillation (same as original)
    logger.log("creating the target model")
    target_model, _ = create_model_and_diffusion(**model_and_diffusion_kwargs)
    ...
    logger.log("starting conditioned consistency training...")
    ConditionedCMTrainLoop(
        model=model,
...
def create_argparser():
    """Create argument parser with radio-specific parameters."""
    defaults = dict(
        data_dir="",
